...
def get_llm_response(system_prompt: str, user_query: str):
    ...
    if 'guia_content' in st.session_state:
        # Se incluye información clave de la guía sobre Prompt Engineering y MML
    ...
    # Reemplazamos el marcador de posición con el contexto de atribuciones
...
def handle_phase_logic(user_prompt: str, user_area: str):
    ...
    # Contexto RAG para simplificar los prompts internos.
    ...
    # ----------------------------------------------------------------------
        ...
        # Esto sucede si el usuario está en un paso intermedio (ej. Diagnostico_Problema_Validacion) y escribe algo distinto a la confirmación.
...
def chat_view(user_name, user_area):
    ...
    if 'current_phase' not in st.session_state:
        # Lógica de inicio mejorada para reflejar las micro-fases
    ...
    if 'area_context' not in st.session_state:
        ...
        if not st.session_state.messages or st.session_state.current_phase == 'inicio':
            ...
            if st.session_state.pat_data.get('problema'):
                 # Definir la siguiente fase si ya hay datos cargados
                 if st.session_state.pat_data.get('componentes_final'):
                     st.session_state.current_phase = 'Fin_MIR'
                     initial_message = f"¡Bienvenido de nuevo! Hemos cargado tu avance hasta la fase de **Componentes**. Ingresa tu siguiente respuesta para validar o avanzar."
                 else:
                     st.session_state.current_phase = 'Componentes_Definicion' if st.session_state.pat_data.get('proposito') is not None else 'Proposito_Definicion'
                     next_phase = st.session_state.current_phase.replace('_', ' ')
                     initial_message = f"""
                     ¡Bienvenido de nuevo! Hemos cargado tu avance hasta la fase de **{next_phase}**. 
                     ...
                     **Propósito Confirmado:** *{st.session_state.pat_data.get('proposito', 'N/A')}*
                     ...
                     Ingresa tu siguiente propuesta para continuar la construcción de la MIR.
                     """
